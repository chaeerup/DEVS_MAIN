   ...
   <div id="wrapper">
   	<div id="main">
   		...
   		<div class="inner">
			...
			<!-- START :: HEADER_CONTENTS -->
		   <section class="container w-75 h-700" style="margin-top: 20px;">      
		      <div class="container">         
		         <div class="messaging">
		            <div class="inbox_msg">
		               ...
		               <div class="inbox_people">      
		                  <div class="headind_srch">
		                     ...
		                     <div class="srch_bar" style="cursor: pointer;">
		               ...
		               <!-- START :: 채팅 메세지 리스트 -->
		                   <div class="mesgs">
		                      ...
		                      <div id="before_enter_chatroom_form" class="mx-auto">
		                         <h1>내 메시지</h1>
		                      ...
		                      <!-- START :: 채팅방 입장시 나오는 폼 -->
		                      <div id="dmRoomOpenForm" style="display: none;">
		                        ...
		                        <div class="type_msg">
		                             <div class="input_msg_write">
		                               ...
		                               <button class="msg_send_btn" id="sendMessage" type="button" onclick="writeChatMessage();"><i class="fa fa-paper-plane-o" aria-hidden="true"></i></button>
		                             </div>
		                        </div>
		                     </div>
		                      <!-- END :: 채팅방 입장시 나오는 폼 -->
		   ...
		   <div id="mask">
		      <div id="searchFriendsForm" class="card rounded-lg">
		         ...
		         <div class="d-flex align-items-center border h-25 overflow-scroll">
		            <strong id="cathMemberList" class="d-flex" style="font-size: 10pt; margin-left: 10px;">TO</strong>
      ...
      $(function(){
           ...
           ws.onmessage=function(event){
               ...
               if (data.enter != undefined) { 
                  ...
                  // 접속자의 메시지가 도착함 해당 채팅방을 열어놓은 상태라면 메시지 출력
...
<!-- START :: chat -->   
   <script type="text/javascript">      
      ...
      function openChatRoom(room_code, thisDiv){
         ...
         // 채팅방 안내 폼 닫기 // 채팅방 폼 열기
         $("#before_enter_chatroom_form").hide();
         $("#dmRoomOpenForm").show();
      }
      ...
      function sendOutChatRoom(){
       ...
       function writeChatMessage(mdata){
         ...
         if(${sessionLoginMember.member_code} === mdata.member_code){
            ...
            img_container
               .append($("<img>").attr({"src":"/resources/images/profileupload/" + mdata.member_img_server_name}))
         ...
         $("input[name='searchFriends']").autocomplete({
            ...
            if(item.image != null){
	            ...
	            li_item.append($("<img>").attr({
	                           "class" : "rounded-circle m-1 bg-white vertical-align-baseline",
            	...
            	li_item.append($("<img>").attr({
                    "class" : "rounded-circle m-1 bg-white vertical-align-baseline",
                    "style" : "width: 30px; height: 30px;",
					"src" : '/resources/images/profileupload/default.png'
                 }))
                 .append($("<span>").attr({"class":"id mx-1"}).text(item.label))
                 .append($("<span>").attr({"class":"name mx-1"}).text(item.name))
...
<!-- START :: 채팅방 생성 -->
   <script type="text/javascript">
      ...
      function addChatMember(code, member_id){
         ...
         $("#cathMemberList").append($("<span>").attr({"class":"card bg-secondary mx-2 text-center", "style":"width: 40px;"}).text(member_id))
   ...
   <script type="text/javascript">
      function fillChatRoomList(data){
         $.each(data, function(index, item){
            ...
            var chat_list = $("<div>").attr({"class":"chat_list", "style":"cursor:pointer;", "data-roomcode":item.room_code, "onclick":"openChatRoom(" + item.room_code + ", this);"});
            ...
            $.each(item.member_list, function(idx, member){      
            	...
            	if(member.member_img_server_name != null) {
	               chat_img.append($("<img>").attr({"class":"rounded-circle", "style":"width: 20px; height: 20px;", "src":"/resources/images/profileupload/" + member.member_img_server_name}))
            	} else {
	               chat_img.append($("<img>").attr({"class":"rounded-circle", "style":"width: 20px; height: 20px;", "src":"/resources/images/profileupload/default.png"}))
            	}
            ...
            if(item.member_list.length >= 3){
               chat_img.empty();
               chat_img.append($("<img>").attr({"class":"rounded-circle", "style" : "width: 40px; height: 40px;", "src":"/resources/images/social/group_chat.png"}))
            }
            
            if(item.member_code != 0){
               chat_ib
                  .append($("<h5>").text(member_id_list))
                  .append($("<span>").text(recent_message_member_id + " - ").append($("<span>").text(item.message))
